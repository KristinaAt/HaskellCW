...
knapsack'' wvs c = table ! c
  where
    ...
    mknapsack 0 = (0, [])
    mknapsack c =
       compareValues [(v + fst(table!(c-w)), n : snd(table!(c-w)))
                       | (n, w, v) <- wvs, w <= c] (0, [])

    compareValues :: [(value, [name])] -> (value, [name]) -> (value, [name])
    compareValues [] result = result
    compareValues ((v1, ns1) : rest) (v2, ns2)
      | v1 > v2   = compareValues rest (v1, ns1)
      | otherwise = compareValues rest (v2, ns2)
...
bknapsack
  ...
  => [(name, weight, value)] -> weight -> (value, [name])
bknapsack wvs c =
  compareValues [ (v + fst(bknapsack (delete (n,w,v)) wvs (c - w)), n : snd(bknapsack (delete (n,w,v) wvs) (c - w))) | (n,w,v) <- allwvs , w <= c ] (0, [])
  where
    compareValues :: [(value, [name])] -> (value, [name]) -> (value, [name])
    compareValues [] result = result
    compareValues ((v1, ns1) : rest) (v2, ns2)
